...
public class AnimeSpecification implements Specification<Anime> {

    private final FilterDTO filter;

    @Override
    public Predicate toPredicate(Root<Anime> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
        ...
        if (StringUtils.hasText(filter.getCountryOrigin())){
            Expression<String> countryOriginToLoweCase = criteriaBuilder.lower(root.get("countryOrigin"));
            Predicate p = criteriaBuilder.like(countryOriginToLoweCase, filter.getCountryOrigin().toLowerCase());
            predicate.add(p);
        }
